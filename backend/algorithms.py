...
def similar_students(userid, college):
    ...
    for application in applications:
        ...
        ap = (
            min(u.num_AP_passed, s.num_AP_passed)
            / max(u.num_AP_passed, s.num_AP_passed)
            if u.num_AP_passed != 0 and s.num_AP_passed != 0
            else 0
        )
...
def recommend_colleges(user_id, colleges):
    ...
    for college in colleges:
        ...
        accepted_ratio = (
            sum_similarity_score_accepted / n_accepted if n_accepted > 0 else 0
        )
        pending_ratio = sum_similary_score_pending / n_pending if n_pending > 0 else 0
        waitlisted_ratio = (
            sum_similary_score_waitlisted / n_waitlisted if n_waitlisted > 0 else 0
        )
        deferred_ratio = (
            sum_similarity_score_deferred / n_deferred if n_deferred > 0 else 0
        )
        withdrawn_ratio = (
            sum_similarity_score_withdrawn / n_withdrawn if n_withdrawn > 0 else 0
        )

        total_similarity = (
            0.7 * accepted_ratio
            + 0.1 * pending_ratio
            + 0.1 * waitlisted_ratio
            + 0.05 * deferred_ratio
            + 0.05 * waitlisted_ratio
        )
...
def similar_hs(hs_name):
    ...
    for hs in HighSchool.objects.all():
        ...
        if u_hs.num_students and hs.num_students:
            max_students = HighSchool.objects.aggregate(Max("num_students"))[
                "num_students__max"
            ]
